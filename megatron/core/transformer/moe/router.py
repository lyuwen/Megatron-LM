...
from megatron.core.fusions.fused_topk_gating import fused_topk_gating
...
class TopKRouter(Router):
    ...
    def seq_aux_loss_load_balancing(self, logits: torch.Tensor, bsz: int, seq_length: int):
        ...
        if (self.config.moe_topk_router_fusion):
            probs, routing_map, tokens_per_expert = fused_topk_gating(logits, self.topk, num_groups=self.config.moe_router_num_groups, group_topk=self.config.moe_router_group_topk, expert_bias=self.expert_bias)
        else:
            probs, routing_map, tokens_per_expert = topk_softmax_with_capacity(
                logits,
                self.topk,
