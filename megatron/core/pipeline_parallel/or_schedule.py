...
def or_pipelining(
    *,
    ...
    ep_num = int(os.environ.get('EP', 1))
    ...
    pipeline_rank_offset = global_rank % ep_num
    if pipeline_rank_offset == 0:
        pipeline_rank = global_rank // ep_num
    print(f'---- numrank = {num_rank} this_rank = {this_rank} global_rank = {global_rank} nmb = {num_microbatches}')
    ...
    B_time = [0.6 for _ in range(num_rank)]
    ...
    # TODO 这个路径记着改一下 改成i相对的
    # TODO 这个路径记着改一下 改成i相对的
    if global_rank == (num_rank-ep_num):
        log_msg = '========================================== ==  =='
        log_msgs.append(log_msg)
        log_msgs.append(log_msg)

    if this_rank == 0:
        print(f'[1f1b] occur_new 1f1b schedule ------ kkkjjjlll------------------------------- ')
    ...
    check_info_path = os.environ.get('CHECK_INFO')
    file_path = check_info_path
    # file_path = '/mnt/public/users/chensi/zj-dev-moe-mfu-0410-new/check_info_8gpus_pp4_ep2_nl28'
    with open(file_path, 'a') as f:
        for msg in log_msgs:
            f.write(msg + '\n')
