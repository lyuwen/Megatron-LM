...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        tokens = torch.empty(
            (args.micro_batch_size, current_seq_length),
            dtype=torch.int64,
        ...
        if args.create_attention_mask_in_dataloader:
            attention_mask = torch.empty(
                (args.micro_batch_size, 1, current_seq_length, current_seq_length),
                dtype=torch.bool,
                device=torch.cuda.current_device(),
            )
        else:
...
# LFu: get_batch_on_this_tp_rank for SFT
def get_batch_on_this_tp_rank_sft(data_iterator, per_seq_average=False):
    args = get_args()
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        labels[labels == tokenizer.pad] = -100
