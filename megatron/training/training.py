...
from megatron.core.rl.entropy import get_batch_entropy
...
def training_log(
    ...
    if iteration % args.log_interval == 0:
        ...
        # Log consumed tokens
        log_string += ' consumed tokens: {:15,d} |'.format(get_consumed_tokens())
        if writer:
            writer.add_scalar('consumed-tokens', get_consumed_tokens(), iteration)
        if wandb_writer:
            wandb_writer.log({'consumed-tokens': get_consumed_tokens()}, iteration)
...
def evaluate_and_print_results(
    ...
    for key in total_loss_dict:
        ...
        if writer:
            ...
            writer.add_scalar(
                '{} validation'.format(group_prefix + key),
                total_loss_dict[key].item(),
                iteration,
                )
            writer.add_scalar(
                '{} validation vs samples'.format(group_prefix + key),
                total_loss_dict[key].item(),
                args.consumed_train_samples,
                )
            if args.log_validation_ppl_to_tensorboard:
                writer.add_scalar(
                    '{} validation ppl'.format(group_prefix + key),
                    ppl,
                    iteration,
                    )
                writer.add_scalar(
                    '{} validation ppl vs samples'.format(group_prefix + key),
                    ppl,
                    args.consumed_train_samples,
                    )
