...
_CONFIG_FOR_DOC = "LlamaMLAConfig"
...
def is_flash_attn_3_available():
    try:
        ...
        if torch.cuda.is_available() and get_device_compute_capability() >= (9, 0) and _NVTE_FLASH_ATTN:
            logger.debug("flash-attn v3 is not installed.")
        return False
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, config: LlamaMLAConfig, device=None):
        super().__init__()
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMLAConfig, layer_idx: int):
        ...
        self.self_attn = ATTENTION_CLASSES.get(config._attn_implementation, DeepseekV3Attention)(
...
class LlamaMLAForCausalLM(LlamaPreTrainedModel, GenerationMixin):
